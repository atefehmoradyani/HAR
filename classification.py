...
# print("td = {}, ct = {}, dte = {}, cte = {}".format(data_train.shape, cat_train.shape, data_test.shape, cat_test.shape))

# SGD = keras.optimizers.SGD(lr=0.01, nesterov=True)
# ADAM = keras.optimizers.Adam(lr=0.0001)
# cat = np.array(cat)
# print("cat shape = ", cat.shape)
input_img = Input(shape=(6000,))
model = Sequential()
model.add(Dense(2000, activation='relu', input_dim=6000))
...
# row_len = len(data)
# train_data = data[0:round(row_len), :]
# train_cat = cat_file[0:round(row_len), :]
# test_data = data[round(row_len):, :]
# test_cat = cat_file[round(row_len):, :]

# row_len = len(data)
# train_data = data[0:round(row_len), :]
# train_cat = cat_file2[0:round(row_len)]
# test_data = data[round(row_len):, :]
# test_cat = cat_file2[round(row_len):]

# model.fit(data, cat_file, validation_split=0.1, batch_size=256, epochs=500, shuffle=True)
model.fit(data_train, cat_train, batch_size=128, epochs=5000, validation_data=(data_test, cat_test), shuffle=True)
# print("==========================")
# model.evaluate(
#     x=data_test,
#     y=cat_test,
...
# NN = MLPClassifier(solver='lbfgs', alpha=1e-5, verbose=1, hidden_layer_sizes=(800, 400), random_state=1).fit(data_train, cat_train)
# pre = NN.predict(data_test)
# print("pre = ".format(pre))
# round(NN.score(data_test, cat_test), 4)
# print(round(NN.score(data_test, cat_test), 4))
...
# p = np.argmax(p, axis=1)
# print("pr = ", p)
# print("cat 0", np.argmax(cat_file[200]))
# p = model.predict(np.reshape(data[600, :], (1, 1000)))
# p = np.argmax(p, axis=1)
# print("pr = ", p)
# print("cat 0", np.argmax(cat_file[600]))

# kmeans = KMeans(n_clusters=50, random_state=0).fit(data)
# km = kmeans.fit_predict(data)
# np.savetxt("kmeans.csv", km, delimiter=',')
# np.savetxt("ategory.csv", cat_file, delimiter=',')

# re = OneVsRestClassifier(LinearSVC(random_state=0)).fit(data_train, cat_train).predict(data_test)
# print(re)
# print("=================")
# print(cat_test)
model.save('classificationucf.h5')